...
**Thành viên thực hiện báo cáo:** [Họ tên TV3] — [MSSV3]
...
| Hình 2.9 | Giao diện Danh sách cuộc trò chuyện |
| Hình 2.10 | Giao diện Tạo nhóm |
| Hình 2.11 | Giao diện Quản lý thành viên nhóm |
| Hình 2.12 | Giao diện Trang chủ (Tabs) |
...
| **Message Broker** | Apache Artemis | 2.53.0 | STOMP protocol, tích hợp Spring WebSocket |
...
### UC-15: Quản lý thành viên nhóm

| Thành phần | Mô tả |
|------------|-------|
| **Tên UC** | Thêm/xóa thành viên, phân quyền admin |
| **Actor** | Admin nhóm |
| **Tiền điều kiện** | User là admin của nhóm |
| **Hậu điều kiện** | Thành viên được thêm/xóa, quyền được cập nhật |
| **Luồng chính** | Thêm: `POST /groups/members/` → add ChatRoomMember. Xóa: `DELETE /groups/members/{userId}/`. Toggle admin: `PATCH /groups/members/{userId}/admin`. |
| **Luồng ngoại lệ** | Non-admin → 403 Forbidden. Duplicate member → 409. |
...
*Hình 2.10: Biểu đồ tuần tự — Rời nhóm & Giải tán nhóm*
...
*Bảng 2.3: Danh sách STOMP events — Module Nhóm*
...
*Bảng 2.2: API Endpoints — Module Danh sách chat & Nhóm*
...
*Hình 2.9: Biểu đồ hoạt động — Luồng tạo nhóm chat*
...
### 3.3.5 Rời nhóm & Giải tán nhóm

- **Rời nhóm**: `DELETE /api/v1/chatrooms/{roomId}/groups/leave/` → xóa ChatRoomMember → xóa khỏi ChatRoom.members. Nếu admin cuối → chuyển quyền.
...
*Bảng 3.3: Đánh giá kiến trúc module Danh sách chat & Nhóm*
...
*Bảng 3.4: Phân tích hiệu năng module Nhóm*
...
> *Kết thúc Báo cáo Thành viên 3 — Danh sách chat & Chat nhóm*
